...
\begin{figure}
	\includegraphics[width=0.8\textwidth]{{spd_N-1e6.00_t-3_ic-hotplanck}.pdf}
...
\begin{figure}
	\includegraphics[width=0.8\textwidth]{{spd_N-1e6.00_t-3_ic-hotplanck_Z-0.10}.pdf}
...
\begin{figure}
	\includegraphics[width=0.8\textwidth]{{spd_N-1e5.04_t-100_ic-hotplanck_Z-1.10}.pdf}
...
We first take $c = 0.1$, leaving $b=0$ in \eqref{ext-dri}. A spurious 
difficulty arises for nonzero $c$, as the given diffusion and drift 
coefficients diverge near the origin. To remedy this, we implement a cutoff 
$x_0$ in the simulation, which we take to be $0.1$ (i.e.\;1\% of the relevant 
frequency range), where the extra diffusion term is taken to go linearly to 
zero as $x$ goes below this cutoff, as it is nonphysical for the activity to 
remain the same all the way to zero frequency anyway.
...
frequencies.
\begin{figure}
	\includegraphics[width=0.8\linewidth]{{spd_N-1e5.70_t-100_ic-planck_c-0.1000_brems}.pdf}
...
\begin{figure}
	\includegraphics[width=0.8\linewidth]{{temp_N-1e5.70_t-100_ic-planck_c-0.1000_brems}.pdf}
...
\begin{figure}
	\centering
	\includegraphics[width=0.8\linewidth]{{spd_N-1e4.86_t-100_ic-planck_b-0.10_k-0.40_Z-0.73_brems}.pdf}
...
single-photon process. The proposed extension \eqref{extk} is parameterized by 
the values $b,c$ and $k$, such that $b=c=0$ corresponds to the standard 
...
which leads by applying the steps \eqref{sumj}--\eqref{5o} to the It\^o process for the $i$-th photon
 \begin{equation} \label{i-3dsde}
 	\dot{\mbf{k_i}}_t = \nabla_{\vb{k}}{D}({\mbf{k_i}_t}) -\beta D(\vb{k_i}_t)\,\nabla_{\vb{k}} U (\vb{k_i}_t)(1+n_{\vb{k_i}_t}(\vb{K})) + \sqrt{2D(\mbf{{k_i}_t})}\,\mbf{\Xi}_t	
 \end{equation}
